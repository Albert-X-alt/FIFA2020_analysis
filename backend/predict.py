...
with torch.no_grad():
    for x, y in loader:
        ...
        output = model(x)
        i+=1
        print(f"Predicted value for the last row: {(output.item()*dataset.target_std+dataset.target_mean)}")
        print(f"True value for the last row: {(y.item()*dataset.target_std+dataset.target_mean)}")
        if(i==20):
            break
